...
<td class="warning" rowspan="1" colspan="2" align="center"><b>Early Check in</b></td>
	...
	?>
		...
		$transd=$this->Madmin->total_trans($ok->id);  //pemasukakn
	//	$casd=$this->Madmin->total_cas($ok->id);
	////harga semua total kamar
	$totkam=$this->Madmin->total_allkamar($ok->id); //tagihan
	$totkamnodisc=$this->Madmin->total_allkamarnodisc($ok->id); //tagihana
	//
	$totbed=$this->Madmin->total_allbed($ok->id); ///tagiahan bed
	//
	//
	$totot=$this->Madmin->total_allot($ok->id); ///tagihan overtime
	//
	//
	$try=$this->Madmin->total_allerly($ok->id); ///tagiahan realy
		...
		///////GUEST ==============================================================================================
           ...
           // echo  $totkam;
			...
			////////20180403
                ...
                $disc_ok;
               $bkar=$ok->b_kartu;
                $bka=$disc_ok*($bkar/100);
                //echo $totalall;
                 $disc_ok=$disc_ok+$bka; ///biayakartu=amount+(amount*persen)
