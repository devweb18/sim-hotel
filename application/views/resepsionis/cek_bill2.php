  ...
  <div class="col-sm-6 col-md-12">
	...
	?>
		...
		<td> <p align="right"><?=number_format($dep->transfer,0,',','.')?>
			<?php 
			//if(empty($dep->transfer) AND $dep->transfer != NULL OR !empty($dep->cas))
			//if($dep->transfer == 0 AND $dep->cas == 0)
			//{ ?>
...
<div class="input-group">
  ...
  <input type="text" class="form-control" name="cas_t" placeholder="Masukkan angka" value="<?=$dep->transfer?>" autofocus>
	...
	<?php	//} ?>
		...
		<td ><p align="right">
		<?php //if(!empty($dep->cas)){
			echo number_format($dep->cas,0,',','.');
		//} ?>
		<?php 
		//if(empty($dep->cas) AND $dep->cas!=NULL OR !empty($dep->transfer)){ 
		//if($dep->cas == 0  AND $dep->transfer ==0)
		//{ ?>
